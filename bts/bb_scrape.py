...
def robot_links()->dict:
    """
    robot_links returns all the battlebots from 2015-2021 and their respective links
    ...
    for i, v in dic.items():
        ...
        if v != "#":
            ...
            for temp in soup.find_all("strong"):          
                ...
                #convert the below to a function that processes val
                if val != None and type(val) == list:
                    if any(":" in x for x in val):
                        ...
                        val = val2
                dict_info[key] = val
            #apply clean function before appending to list (see bb-create-db)
...
#dump_to(dict_links, "bb-links.json")
#read from local json file (vs from web request)
with open("json/bb-links.json", "r") as f:
    dict_links = load(f)
